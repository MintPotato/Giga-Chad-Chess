...
class Board:
    def __init__(self, only_first_solution=False):
        ...
        for pair in range(self.K):
            ...
            self.board = self.figure().figure_attack(self.board, i, j, self.N)
            self.board[i][j] -= 1
            coords_placed += [f'({i}, {j})']
    ...
    def solves(self, board: list[list], placed: int, file, i1: int, j1: int, coords: list[str]):
            ...
            for i in range(i1, self.N):
                for j in range(j1, self.N):
                    ...
                    if board[i][j] == 0:
                        ...
                        updated_board = self.figure().figure_attack(board, i, j, self.N)
        ...
        # Если выставленно необходимое количество фигур увеличивается счетчик найденных решений и осуществляется запись полученых координат в 'output.txt'
            ...
            if not self.first_founded:
                self.board_to_console(board)
                self.first_founded = True

    def board_to_console(self, board: list[list]):
        '''
        Функция выводяющая первое найденное решение в консоль

        Параметры:
            board: list[list]
                Матрица доски с найденным решением
        '''
